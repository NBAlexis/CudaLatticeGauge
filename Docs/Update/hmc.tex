...
Is the action for a particular $U_{\mu}(n)$. $\Sigma$ is the \index{staple}`staple'. The staple for $U_{\mu}(n)$ is independent of $U_{\mu}(n)$, denoting
\begin{equation}
\begin{split}
...
Eq.~(\ref{eq.hmc.force}) has a different sign with Eqs.~(2.55) and (2.67) of Ref.~\cite{latticeqcdbook2017}, but the sign is same as Eq.~(8.42) of Ref.~\cite{latticeqcdbook2010} (Note that in Ref.~\cite{latticeqcdbook2010}, $F\equiv -\dot{P}$ but we define $F\equiv \dot{P}$). But in Eq.~(8,42), the $\{\ldots\}_{TA}$ is different.
...
If the hybrid Monte Carlo can be implemented exactly, then, when \index{equilibrium}equilibrium is reached, $H$ should be unchanged, so, in some implementation, the Metropolis step can be ignored to archive a better accept rate.
