...
\begin{equation}
\begin{split}
&{\rm Im}\left[L^{\dagger}TUR\right]={\rm Im}\left[\sum _{\alpha,\beta,\rho}L_{\alpha}^*T_{\alpha\beta}U_{\beta\rho}V_{\rho}\right]={\rm Im}\left[\sum _{\alpha,\beta,\rho}T_{\alpha\beta}U_{\beta\rho}V_{\rho}L_{\alpha}^*\right]={\rm Im}\left[{\rm tr}\left[TU(VL^{\dagger})\right]\right]\\
\end{split}
\end{equation}
...
\For{$i=0$ to $r$}
    ...
    \For{$j=0$ to $itera$}
        ...
        \EndIf
        \State ${\bf t}=A{\bf s}$
        \State $\omega = |{\bf t}^*\cdot {\bf s}|/\|{\bf t}\|$
...
\EndFor
\end{algorithmic}
\caption{\label{alg.BiCGStab}BiCGStab}
\end{algorithm}
