...
&=-\frac{\beta}{N} i{\rm Im\;tr}\left[T_aU_{\mu}(n)\Sigma ^{\dagger}_{\mu}(n)\right]\\
\end{split}
\end{equation}
...
\label{eq.hmc.force}
\end{equation}
Note that, $\{U_{\mu}(n)\Sigma ^{\dagger}_{\mu}(n)\}_{TA}=-2i\sum _aT_a{\rm Im}[tr[T_a\cdot W]]$, so $\dot{\omega}_a=\frac{\beta}{N}{\rm Im}[tr[T_a\cdot W]]$ is still a \textbf{real} number.
...
\end{split}
\end{equation}
with $C=m_f+\left(4/a\right)$ and $\kappa=1/(2am_f+8)$.
