...
\end{split}
\end{equation}
Note that, in Ref.~\cite{staggeredRHMC} the fourth root is just to remove doublers, so for $N_f=2$, it is $A^{1/2}$ in spite that $A=D^{\dagger}D$ already considered $N_f=2$.
However, it should not be correct.
\textcolor[rgb]{1,0,0}{\textbf{According to Ref.~\cite{Wong:2007uz}, $A$ should bring another doubler, so it should be $N_f/8$, or, one can use a even (or odd) pseudofermion to remove another doubler.}}
