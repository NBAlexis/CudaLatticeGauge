...
void CMeasurePolyakov::OnConfigurationAcceptedSingleField(const class CFieldGauge* pAcceptGauge, const class CFieldGauge* pCorrespondingStaple)
{
    ...
    _LAUNCH_KERNEL(_kernelPolyakovTraceOfSite, block1, threads1, m_pTmpLoop, m_pTraceRes, m_pTmpDeviceSum);

    _LAUNCH_KERNEL(_kernelInitialStaticPotentialCorrelatorOfSite, block2, threads2, m_pCorrelatorCounter, m_pCorrelator);

    //========= It is impossible to calculate every pairs of sites
    //_LAUNCH_KERNEL(_kernelStaticPotentialCorrelatorOfSite, block1, threads1, 
    //    m_pTraceRes,
    ...
    _LAUNCH_KERNEL(_kernelStaticPotentialCorrelatorOfSite2, block3, threads3, 
        m_pTraceRes,
        m_uiMaxLengthSq,
        m_pCorrelatorCounter,
        m_pCorrelator
    ...
    _LAUNCH_KERNEL(_kernelAverageStaticPotentialCorrelatorOfSite, block2, threads2, m_pCorrelatorCounter, m_pCorrelator);
...
{
    Average();
    appAssert(static_cast<UINT>(m_uiConfigurationCount * m_lstR.Num())
