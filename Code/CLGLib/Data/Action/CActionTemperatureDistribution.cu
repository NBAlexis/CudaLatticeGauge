...
static __device__ __inline__ DOUBLE _deviceSimplePower(DOUBLE v, INT iPower)
{
    if (iPower < 0)
    {
        if (v > _CLG_FLT_MIN_)
...
/**
* for i = 0,1,2
* calculate kappa boson[n]^m ((boson[n+i]-boson[n])^2 + (boson[n]-boson[n-i])^2) /2
*/
__global__ void _CLG_LAUNCH_BOUND
_kernel_KinematicBoson(const Real* __restrict__ boson,
    ...
    BYTE byFieldId)
{
    ...
    //if (sIdx.IsDirichlet())
    //{
    //    resbuffer[uiSiteIndex] = 0.0;
    //    return;
    //}

    DOUBLE result = 0.0;
    const DOUBLE thissite = boson[uiSiteIndex];
    //idir = mu
    for (UINT idir = 0; idir < 4; ++idir)
    {
        ...
        {
            DOUBLE delta = boson[x_p_mu_Boson.m_uiSiteIndex] - thissite;
            result += delta * delta;
            delta = thissite - boson[x_m_mu_Boson.m_uiSiteIndex];
            result += delta * delta;
        }
    }
    resbuffer[uiSiteIndex] = _deviceSimplePower(thissite, xiPower) * result * fKappa * 0.5;
}

/**
* m b(x)^(m-1) ((b(x+mu)-b(x))^2 + (b(x)-b(x-mu))^2)/2 
+ b(x)^m (2b(x)-b(x+mu)-b(x-mu)) 
+ b(x-mu)^m (b(x)-b(x-mu)) - b(x+mu)^m (b(x+mu)-b(x))
*/
__global__ void _CLG_LAUNCH_BOUND
_kernel_KinematicBosonForce(
    const Real* __restrict__ boson,
    const SIndex* __restrict__ pBosonMove,
    Real* forceBuffer,
    DOUBLE fKappa,
    INT xiPower,
    BYTE byFieldId)
{
    ...
    DOUBLE result = 0.0;
    const DOUBLE thissite = boson[uiSiteIndex];
    const DOUBLE thissite_mpower = _deviceSimplePower(thissite, xiPower);
    //idir = mu
    for (UINT idir = 0; idir < 4; ++idir)
    {
        //x, mu
        ...
        const DOUBLE n_p_mu_value = boson[x_p_mu_Boson.m_uiSiteIndex];
        const DOUBLE n_m_mu_value = boson[x_m_mu_Boson.m_uiSiteIndex];
        const DOUBLE deltap = n_p_mu_value - thissite;
        const DOUBLE deltam = thissite - n_m_mu_value;
        ...
        //if (!x_p_mu_Boson.IsDirichlet())
        {
            ...
            result += 0.5 * xiPower * thissite_mpower * (deltap * deltap + deltam * deltam) / thissite;
            result += (deltam - deltap) * thissite_mpower;
        }

        //if (!x_m_mu_Boson.IsDirichlet())
        {
            
            result += deltam * _deviceSimplePower(n_m_mu_value, xiPower);
            result -= deltap * _deviceSimplePower(n_p_mu_value, xiPower);
        }
    }
    result = result * fKappa;
    forceBuffer[uiSiteIndex] -= result;
}
...
void CActionTemperatureDistribution::Initial(class CLatticeData* pOwner, const CParameters& param, BYTE byId)
{
    ...
    param.FetchValueINT(_T("XiPower"), m_iXiPower);
}

DOUBLE CActionTemperatureDistribution::Energy(UBOOL bBeforeEvolution, INT gaugeNum, INT bosonNum, INT tensor2Num, const CFieldGauge* const* gaugeFields, const CFieldBoson* const* bosonFields, const CFieldTensor2* const* tensor2Fields, const CFieldGauge* const* stapleFields)
{
    if (bBeforeEvolution)
    {
    ...
    m_fLastEnergy = (8.0 + m_fM) * phi2 + m_fLambda * phi4 - partiald.x;
        */
        ...
        //dboson->Return();
        _LAUNCH_KERNEL(_kernel_KinematicBoson, block, threads,
            pBoson->m_pDeviceData,
            appGetLattice()->m_pIndexCache->m_pMoveCache[pBoson->m_byFieldId],
            _D_RealThreadBuffer,
            m_fBosonKinetic,
            m_iXiPower,
            pBoson->m_byFieldId);
    ...
    CFieldGauge* const* stapleFields, ESolverPhase ePhase) const
{
    ...
    if (abs(m_fBosonKinetic) > _CLG_FLT_MIN_)
    {
        ...
        //dboson->Return();
        _LAUNCH_KERNEL(_kernel_KinematicBosonForce, block, threads,
            pBoson->m_pDeviceData,
            appGetLattice()->m_pIndexCache->m_pMoveCache[pBoson->m_byFieldId],
            pBosonForce->m_pDeviceData,
            m_fBosonKinetic * 0.5,
...
void CActionTemperatureDistribution::PrepareForHMC(INT gaugeNum, INT bosonNum, const CFieldGauge* const* gaugeFields, const CFieldBoson* const* bosonFields, UINT iUpdateIterate)
{
    if (0 == iUpdateIterate)
    {
        m_fLastEnergy = Energy(FALSE, gaugeNum, bosonNum, 0, gaugeFields, bosonFields, NULL, NULL);
    }
}
