...
void CActionFermionKS::Initial(CLatticeData* pOwner, const CParameters& param, BYTE byId)
{
    ...
    m_pFerimionField = dynamic_cast<CFieldFermion*>(appGetLattice()->GetFieldById(static_cast<BYTE>(iFieldId)));
...
DOUBLE CActionFermionKS::Energy(UBOOL bBeforeEvolution, INT gaugeNum, INT bosonNum, INT tensor2Num, const CFieldGauge* const* gaugeFields, const CFieldBoson* const* bosonFields, const CFieldTensor2* const* tensor2Fields, const CFieldGauge* const* stapleFields)
{
    //[ (DD)^(-1/4) phi ]^2
    ...
    //CFieldFermion* pPooled = dynamic_cast<CFieldFermion*>(appGetLattice()->GetPooledFieldById(static_cast<BYTE>(m_pFerimionField->m_byFieldId)));
    //m_pFerimionField->CopyTo(pPooled);
    ...
    //pPooled->Return();
    return m_pFerimionField->Energy(gaugeNum, bosonNum, tensor2Num, gaugeFields, bosonFields, tensor2Fields);
}
