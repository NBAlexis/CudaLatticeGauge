    ...
    UINT bXorY, UBOOL bPlusMu, UBOOL bPlusTau)
{
    ...
    //Can't beleave deviceLink is faster...
    dir1[0] = iMu;
    dir1[1] = iMu;
    dir1[2] = iTau;
    deviceSU3 sRet = _deviceLink(pDeviceData, sStartSite, 3, byFieldId, dir1);
    //deviceSU3 sRet = bPlusMu ? 
    //    (bPlusTau ? _deviceXXT_PP(pDeviceData, sStartSite, byFieldId, 1 - bXorY) : _deviceXXT_PM(pDeviceData, sStartSite, byFieldId, 1 - bXorY))
    //  : (bPlusTau ? _deviceXXT_MP(pDeviceData, sStartSite, byFieldId, 1 - bXorY) : _deviceXXT_MM(pDeviceData, sStartSite, byFieldId, 1 - bXorY));
