//=============================================================================
// FILENAME : CFieldFermionKSSU3REM.cpp
// 
// DESCRIPTION:
// 
//
...
//    const deviceSU3 vnn1 = _deviceLinkEMT(pGauge, pU1, fCharge, sSite4, Llength, 1, Ldirs);
//    const deviceSU3 vnn2 = _deviceLinkEMT(pGauge, pU1, fCharge, sSite4, Rlength, 1, Rdirs);
//
    ...
    Real fRealCoeff, const CLGComplex& cCmpCoeff) const
{
    ...
    {
        //The cached rotation links were built with the U(1) phase of m_fQ
        //(CacheRotationLinkCharge / CacheRotationLinkPhaseFieldId in StapleCache),
        //so the Cached kernel needs no charge/phase arguments.
        CFieldFermionKSTKernelR<deviceSU3Vector, deviceSU3, 3>::DOperatorKS_R_ImaginaryRotation_Cached(
            m_fOmega,
            this->m_bEachSiteEta,
            (deviceSU3Vector*)pTargetBuffer,
            (const deviceSU3Vector*)pBuffer,
            (const deviceSU3*)appGetStapleCache(byGaugeFieldId)->GetRotationBuffer(),
            this->m_byFieldId,
            byGaugeFieldId,
            f2am,
            bDagger,
    ...
    {
        CFieldFermionKSTKernelR<deviceSU3Vector, deviceSU3, 3>::DOperatorKS_R_ImaginaryRotation_EM(
            m_fOmega,
            m_fQ,
            this->m_bEachSiteEta,
            (deviceSU3Vector*)pTargetBuffer,
...
//
//#if 1
//    _LAUNCH_KERNEL(_kernelDFermionKS_PREM_XYTerm, block, threads, 
//        pSource,
//        pGauge,
//        pU1->m_pDeviceData,
...
//#if 1
//
//    _LAUNCH_KERNEL(_kernelDFermionKS_PREM_XYTau_Term, block, threads, 
//        pSource,
//        pGauge,
//        pU1->m_pDeviceData,
//        pTarget,
...
void CFieldFermionKSSU3REM::DOperatorKSOnEvenOrOdd(void* pTargetBuffer, const void* pGaugeBuffer, BYTE byGaugeFieldId, UBOOL bEven, Real f2am,
    UBOOL bDagger, EOperatorCoefficientType eOCT, Real fRealCoeff, const CLGComplex& cCmpCoeff) const
{
    ...
    //Basic KS + EM term (even-odd)
    //REM uses shift-center-half (rotation axis at the center), which is the PlusEta
    //semantics: DOperatorEMEvenOdd dispatches to the PlusEta even-odd kernel when
    //bShiftCenter is TRUE, consistent with the non even-odd DOperatorEM path.
    ...
    {
        //The cached rotation links were built with the U(1) phase of m_fQ,
        //so the Cached even-odd kernel needs no charge/phase arguments.
        CFieldFermionKSTKernelR<deviceSU3Vector, deviceSU3, 3>::DOperatorKSOnEvenOrOdd_R_ImaginaryRotation_Cached(
            m_fOmega,
            m_bEachSiteEta,
            (deviceSU3Vector*)pTargetBuffer,
            (const deviceSU3*)appGetStapleCache(byGaugeFieldId)->GetRotationBuffer(),
    ...
    {
        CFieldFermionKSTKernelR<deviceSU3Vector, deviceSU3, 3>::DOperatorKSOnEvenOdd_R_ImaginaryRotation_EM(
            m_fOmega,
            m_bEachSiteEta,
            (deviceSU3Vector*)pTargetBuffer,
            (const deviceSU3*)pGaugeBuffer,
    ...
    const void* pGaugeBuffer, BYTE byGaugeFieldId) const
{
    ...
    //CFieldFermionKSTKernel<deviceSU3Vector, deviceSU3, 3>::KSForceEM(
    //    (deviceSU3*)pForce, 
    //    (const deviceSU3*)pGaugeBuffer, 
    //    pU1->m_pDeviceData, m_fQ,
    //    m_pRationalFieldPointers,
    //    GRASet.m_pRASet[m_iMDIndex]->m_pDeviceData,
    //    GRASet.m_pRASet[m_iMDIndex]->m_uiDegree,
    //    m_byFieldId);

    CFieldGaugeSU3* pTempForce = dynamic_cast<CFieldGaugeSU3*>(appGetLattice()->GetPooledFieldById(byGaugeFieldId, _T(__FILE__), __LINE__));
    pTempForce->Zero();
    CFieldFermionKSSU3::DerivateD0(pTempForce->m_pDeviceData, pGaugeBuffer, byGaugeFieldId);
    ...
    for (INT imu = 0; imu < 2; ++imu)
    {
        ...
        for (INT pathidx = 0; pathidx < 6; ++pathidx)
        {
            for (INT iSeperation = 0; iSeperation < 4; ++iSeperation)
            {
                ...
                _LAUNCH_KERNEL(_kernelDFermionKSForce_PREM_XYTerm, block, threads, 
                    (const deviceSU3*)pGaugeBuffer,
                    pU1->m_pDeviceData,
                    (deviceSU3*)pForce,
                    appGetLattice()->m_pIndexCache->m_pEtaMu,
        ...
        for (INT isixtype = 0; isixtype < 6; ++isixtype)
        {
            ...
            */
            ...
            for (INT iSeperation = 0; iSeperation < 4; ++iSeperation)
            {
                ...
                if (bHasLeft || bHasRight)
                {
                    _LAUNCH_KERNEL(_kernelDFermionKSForce_PREM_XYTau_Term, block, threads, 
                        (const deviceSU3*)pGaugeBuffer,
                        pU1->m_pDeviceData,
                        (deviceSU3*)pForce,
                        m_pRationalFieldPointers,
    ...
    CFieldFermionKSTKernelR<deviceSU3Vector, deviceSU3, 3>::DerivateD0_REM(
        m_fOmega, 
        m_bEachSiteEta,
        m_fQ,
        m_pDeviceData, 
        m_byFieldId, 
        pTempForce->m_pDeviceData,
        (const deviceSU3*)pGaugeBuffer,
        pU1->m_pDeviceData,
        byGaugeFieldId, 
    ...
    pTempForce->ApplyPhaseR(pU1, -m_fQ);
    CCommonKernelField<deviceSU3>::AxpyPlus((deviceSU3*)pForce, _HC_Volume * _HC_Dir, pTempForce->m_pDeviceData);
    pTempForce->Return();
}

void CFieldFermionKSSU3REM::CalculateForceEvenOddS(const CFieldGauge* pGauge, CFieldGauge* pForce, ESolverPhase ePhase) const
{
    const CFieldGaugeU1Real* pU1 = dynamic_cast<const CFieldGaugeU1Real*>(appGetLattice()->GetFieldById(m_byEMFieldID));
    if (NULL == pU1)
    {
    ...
    //Base class computes the standard KS even-odd force (AddConnection per rational
    //term) and calls CalculateForceEvenOddS_SingleTermOfRational (overridden below)
    //which adds the rotation + EM even-odd force.
    CFieldGauge* thisf0 = dynamic_cast<CFieldGauge*>(appGetLattice()->GetPooledFieldById(pForce->m_byFieldId, _T(__FILE__), __LINE__));
    thisf0->Zero();
    CFieldFermionKSSU3::CalculateForceEvenOddS(pGauge, thisf0, ePhase);
    thisf0->ApplyPhaseR(pU1, -m_fQ);
    pForce->AxpyPlus(thisf0);
    thisf0->Return();
}

void CFieldFermionKSSU3REM::CalculateForceEvenOddS_SingleTermOfRational(const CFieldGauge* pGauge, const CFieldFermionKS* phi, CFieldGauge* pForce, Real fCoef, INT iRationalTermIndex) const
{
    //Basic KS even-odd force (AddConnection) comes from the base class
    CFieldFermionKSSU3::CalculateForceEvenOddS_SingleTermOfRational(pGauge, phi, pForce, fCoef, iRationalTermIndex);

    //Rotation + EM even-odd force
    //phi is the packed field with phi on even sites and D0 phi on odd sites,
    //so split it into phi (odd zeroed) and D0 phi (even zeroed) for the kernels.
    const CFieldGaugeU1Real* pU1 = dynamic_cast<const CFieldGaugeU1Real*>(appGetLattice()->GetFieldById(m_byEMFieldID));
    if (NULL == pU1)
    {
        appCrucial(_T("Background field cannot be found!\n"));
    ...
    deviceSU3Vector* hostPointers[2] = {
        (deviceSU3Vector*)(pPhiOnly->GetData()),
        (deviceSU3Vector*)(pD0Only->GetData())
    };
    deviceSU3Vector** pPointers = CRationalFieldPointer::GetInstance()->GetRationPoint<deviceSU3Vector>(2);
    appSimpleCopyHD(pPointers, hostPointers, sizeof(deviceSU3Vector*) * 2);

    CFieldFermionKSTKernelR<deviceSU3Vector, deviceSU3, 3>::DerivateD0_REMOnEvenOdd(
        m_fOmega,
        m_bEachSiteEta,
        m_fQ,
        this->m_pDeviceData,
        this->m_byFieldId,
        (deviceSU3*)(pForce->GetData()),
        (const deviceSU3*)(pGauge->GetData()),
        pU1->m_pDeviceData,
        pGauge->m_byFieldId,
        pPointers,
        GRASet.m_pRASet[this->m_iMDIndex]->m_pDeviceData + iRationalTermIndex,
        1);
...
void CFieldFermionKSSU3REM::InitialOtherParameters(CParameters& params)
{
    ...
    INT iValue = 0;
    if (params.FetchValueINT(_T("CachedGauge"), iValue))
    {
        m_byUseCachedGauge = static_cast<BYTE>(iValue);
    }
...
void CFieldFermionKSSU3REM::ApplyGammaS(const CFieldGauge* pGauge, EGammaMatrix eGamma)
{
    const CFieldGaugeU1Real* pU1 = dynamic_cast<const CFieldGaugeU1Real*>(appGetLattice()->GetFieldById(m_byEMFieldID));
    if (NULL == pU1)
    {
    ...
    _FermionKernelGamma::appApplyGammaKSEM(
        m_pDeviceData,
...
void CFieldFermionKSSU3REM::CopyParamTo(CField* U) const
{
    ...
    pTarget->m_byUseCachedGauge = m_byUseCachedGauge;
}
