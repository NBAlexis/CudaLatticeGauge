...
__global__ void _CLG_LAUNCH_BOUND
_kernelKSApplyGamma1234EvenOdd(
    deviceSU3Vector* pMe,
    const deviceSU3* __restrict__ pGauge,
    const SIndex* __restrict__ pGaugeMove,
    const SIndex* __restrict__ pFermionMove,
    ...
    BYTE byDir)
{
    ...
    deviceSU3Vector result = x_Gauge_element.MulVector(pMe[x_p_mu_Fermion.m_uiSiteIndex]);
    if (x_p_mu_Fermion.NeedToOpposite())
    {
        result.MulReal(F(-1.0));
    }
    if (x_m_mu_Fermion.NeedToOpposite())
    {
        result.Sub(x_m_mu_Gauge_element.MulVector(pMe[x_m_mu_Fermion.m_uiSiteIndex]));
    }
    else
    {
        result.Add(x_m_mu_Gauge_element.MulVector(pMe[x_m_mu_Fermion.m_uiSiteIndex]));
    }
    //result.MulReal(eta_mu);
...
__global__ void _CLG_LAUNCH_BOUND
_kernelKSApplyGammaEta1234EvenOdd(
    deviceSU3Vector* pMe,
    const deviceSU3* __restrict__ pGauge,
    const SIndex* __restrict__ pGaugeMove,
    const SIndex* __restrict__ pFermionMove,
...
__global__ void _CLG_LAUNCH_BOUND
_kernelFermionKSForceGamma1234EvenOdd(
    deviceSU3* pForce,
    BYTE idir,
    Real fCoeff,
    BYTE byFieldId)
{
    ...
    //x, mu
    const UINT linkIndex = _deviceGetLinkIndex(uiSiteIndex, idir);
    deviceSU3 f0 = pForce[linkIndex];
    f0.MulComp(_make_cuComplex(F(0.0), fCoeff));
    pForce[linkIndex].Sub(f0);
}
    ...
    const SCHAR* __restrict__ path)
{
    ...
    for (BYTE iSeperation = 0; iSeperation <= 2; ++iSeperation)
    {
        ...
        if (bHasLeft || bHasRight)
        {
            ...
            const deviceSU3 vnn1 = _deviceLinkT(pGauge, sSite4, LLength, byGaugeFieldId, pathLeft);
            const deviceSU3 vnn2 = _deviceLinkT(pGauge, sSite4, RLength, byGaugeFieldId, pathRight);
...
/**
* The applied gamma is 2a x gamma
* for gamma1,2,3,4 and gamma51, 52, 53, 54, it is actually (i x gamma)
*/
...
void CFieldFermionKSSU3Gamma::appApplyGammaKSEvenOdd(
    void* pTargetBuffer,
    const void* pGaugeBuffer,
    EGammaMatrix eGamma,
    UBOOL bShiftCenter,
    ...
    BYTE byGaugeFieldID)
{
    ...
    case GAMMA4:
    {
        ...
        {
            _kernelKSApplyGamma1234EvenOdd << <block, threads >> > (
                pTarget,
                pGauge,
                appGetLattice()->m_pIndexCache->m_pGaugeMoveCache[byFieldID],
                appGetLattice()->m_pIndexCache->m_pMoveCache[byFieldID],
    ...
    Real fRealCoeff, const CLGComplex& cCmpCoeff) const
{
    ...
    if (abs(m_fCoeffGamma1) > _CLG_FLT_EPSILON)
    {
        ...
        _FermionKernelGamma::appApplyGammaKS(pTargetBuffer, pBuffer, pGaugeBuffer, GAMMA1, m_bEachSiteEta, bDagger, m_fCoeffGamma1, eOCT, fRealCoeff, cCmpCoeff, m_byFieldId, byGaugeFieldId);
    }
    if (abs(m_fCoeffGamma2) > _CLG_FLT_EPSILON)
    {
        _FermionKernelGamma::appApplyGammaKS(pTargetBuffer, pBuffer, pGaugeBuffer, GAMMA2, m_bEachSiteEta, bDagger, m_fCoeffGamma2, eOCT, fRealCoeff, cCmpCoeff, m_byFieldId, byGaugeFieldId);
    }
    if (abs(m_fCoeffGamma3) > _CLG_FLT_EPSILON)
    {
        _FermionKernelGamma::appApplyGammaKS(pTargetBuffer, pBuffer, pGaugeBuffer, GAMMA3, m_bEachSiteEta, bDagger, m_fCoeffGamma3, eOCT, fRealCoeff, cCmpCoeff, m_byFieldId, byGaugeFieldId);
    }
    if (abs(m_fCoeffGamma4) > _CLG_FLT_EPSILON)
    {
        _FermionKernelGamma::appApplyGammaKS(pTargetBuffer, pBuffer, pGaugeBuffer, GAMMA4, m_bEachSiteEta, bDagger, m_fCoeffGamma4, eOCT, fRealCoeff, cCmpCoeff, m_byFieldId, byGaugeFieldId);
    }

    if (abs(m_fCoeffSigma12) > _CLG_FLT_EPSILON)
    {
        _FermionKernelGamma::appApplyGammaKS(pTargetBuffer, pBuffer, pGaugeBuffer, SIGMA12, m_bEachSiteEta, bDagger, m_fCoeffSigma12, eOCT, fRealCoeff, cCmpCoeff, m_byFieldId, byGaugeFieldId);
    }
    if (abs(m_fCoeffSigma13) > _CLG_FLT_EPSILON)
    {
        _FermionKernelGamma::appApplyGammaKS(pTargetBuffer, pBuffer, pGaugeBuffer, SIGMA31, m_bEachSiteEta, bDagger, m_fCoeffSigma13, eOCT, fRealCoeff, cCmpCoeff, m_byFieldId, byGaugeFieldId);
    }
    if (abs(m_fCoeffSigma14) > _CLG_FLT_EPSILON)
    {
        _FermionKernelGamma::appApplyGammaKS(pTargetBuffer, pBuffer, pGaugeBuffer, SIGMA41, m_bEachSiteEta, bDagger, m_fCoeffSigma14, eOCT, fRealCoeff, cCmpCoeff, m_byFieldId, byGaugeFieldId);
    }
    if (abs(m_fCoeffSigma23) > _CLG_FLT_EPSILON)
    {
        _FermionKernelGamma::appApplyGammaKS(pTargetBuffer, pBuffer, pGaugeBuffer, SIGMA23, m_bEachSiteEta, bDagger, m_fCoeffSigma23, eOCT, fRealCoeff, cCmpCoeff, m_byFieldId, byGaugeFieldId);
    }
    if (abs(m_fCoeffSigma24) > _CLG_FLT_EPSILON)
    {
        _FermionKernelGamma::appApplyGammaKS(pTargetBuffer, pBuffer, pGaugeBuffer, SIGMA42, m_bEachSiteEta, bDagger, m_fCoeffSigma24, eOCT, fRealCoeff, cCmpCoeff, m_byFieldId, byGaugeFieldId);
    }
    if (abs(m_fCoeffSigma34) > _CLG_FLT_EPSILON)
    {
        _FermionKernelGamma::appApplyGammaKS(pTargetBuffer, pBuffer, pGaugeBuffer, SIGMA43, m_bEachSiteEta, bDagger, m_fCoeffSigma34, eOCT, fRealCoeff, cCmpCoeff, m_byFieldId, byGaugeFieldId);
    }

    if (abs(m_fCoeffGamma51) > _CLG_FLT_EPSILON)
    {
        _FermionKernelGamma::appApplyGammaKS(pTargetBuffer, pBuffer, pGaugeBuffer, GAMMA51, m_bEachSiteEta, bDagger, m_fCoeffGamma51, eOCT, fRealCoeff, cCmpCoeff, m_byFieldId, byGaugeFieldId);
    }
    if (abs(m_fCoeffGamma52) > _CLG_FLT_EPSILON)
    {
        _FermionKernelGamma::appApplyGammaKS(pTargetBuffer, pBuffer, pGaugeBuffer, GAMMA52, m_bEachSiteEta, bDagger, m_fCoeffGamma52, eOCT, fRealCoeff, cCmpCoeff, m_byFieldId, byGaugeFieldId);
    }
    if (abs(m_fCoeffGamma53) > _CLG_FLT_EPSILON)
    {
        _FermionKernelGamma::appApplyGammaKS(pTargetBuffer, pBuffer, pGaugeBuffer, GAMMA53, m_bEachSiteEta, bDagger, m_fCoeffGamma53, eOCT, fRealCoeff, cCmpCoeff, m_byFieldId, byGaugeFieldId);
    }
    if (abs(m_fCoeffGamma54) > _CLG_FLT_EPSILON)
    {
        _FermionKernelGamma::appApplyGammaKS(pTargetBuffer, pBuffer, pGaugeBuffer, GAMMA54, m_bEachSiteEta, bDagger, m_fCoeffGamma54, eOCT, fRealCoeff, cCmpCoeff, m_byFieldId, byGaugeFieldId);
    }

    if (abs(m_fCoeffGamma5) > _CLG_FLT_EPSILON)
    {
        _FermionKernelGamma::appApplyGammaKS(pTargetBuffer, pBuffer, pGaugeBuffer, GAMMA5, m_bEachSiteEta, bDagger, m_fCoeffGamma5, eOCT, fRealCoeff, cCmpCoeff, m_byFieldId, byGaugeFieldId);
    }
}

void CFieldFermionKSSU3Gamma::DOperatorKSOnEvenOrOdd(void* pTargetBuffer, const void* pGaugeBuffer, BYTE byGaugeFieldId, UBOOL bEven, Real f2am,
    UBOOL bDagger, EOperatorCoefficientType eOCT, Real fRealCoeff, const CLGComplex& cCmpCoeff) const
{
    CFieldFermionKSSU3::DOperatorKSOnEvenOrOdd(pTargetBuffer, pGaugeBuffer, byGaugeFieldId, bEven, f2am, bDagger, eOCT, fRealCoeff, cCmpCoeff);

    if (abs(m_fCoeffGamma1) > _CLG_FLT_EPSILON)
    {
        _FermionKernelGamma::appApplyGammaKSEvenOdd(pTargetBuffer, pGaugeBuffer, GAMMA1, m_bEachSiteEta, bEven, bDagger, m_fCoeffGamma1, eOCT, fRealCoeff, cCmpCoeff, m_byFieldId, byGaugeFieldId);
    }
    if (abs(m_fCoeffGamma2) > _CLG_FLT_EPSILON)
    {
        _FermionKernelGamma::appApplyGammaKSEvenOdd(pTargetBuffer, pGaugeBuffer, GAMMA2, m_bEachSiteEta, bEven, bDagger, m_fCoeffGamma2, eOCT, fRealCoeff, cCmpCoeff, m_byFieldId, byGaugeFieldId);
    }
    if (abs(m_fCoeffGamma3) > _CLG_FLT_EPSILON)
    {
        _FermionKernelGamma::appApplyGammaKSEvenOdd(pTargetBuffer, pGaugeBuffer, GAMMA3, m_bEachSiteEta, bEven, bDagger, m_fCoeffGamma3, eOCT, fRealCoeff, cCmpCoeff, m_byFieldId, byGaugeFieldId);
    }
    if (abs(m_fCoeffGamma4) > _CLG_FLT_EPSILON)
    {
        _FermionKernelGamma::appApplyGammaKSEvenOdd(pTargetBuffer, pGaugeBuffer, GAMMA4, m_bEachSiteEta, bEven, bDagger, m_fCoeffGamma4, eOCT, fRealCoeff, cCmpCoeff, m_byFieldId, byGaugeFieldId);
    }
}

void CFieldFermionKSSU3Gamma::CalculateForceEvenOddS(const CFieldGauge* pGauge, CFieldGauge* pForce, ESolverPhase ePhase) const
{
    CFieldFermionKSSU3::CalculateForceEvenOddS(pGauge, pForce, ePhase);

    if (abs(m_fCoeffGamma1) > _CLG_FLT_EPSILON)
    {
        _FermionKernelGamma::GammaKSForceEvenOdd(pForce->GetData(), m_fCoeffGamma1, GAMMA1, m_byFieldId);
    }
    if (abs(m_fCoeffGamma2) > _CLG_FLT_EPSILON)
    {
        _FermionKernelGamma::GammaKSForceEvenOdd(pForce->GetData(), m_fCoeffGamma2, GAMMA2, m_byFieldId);
    }
    if (abs(m_fCoeffGamma3) > _CLG_FLT_EPSILON)
    {
        _FermionKernelGamma::GammaKSForceEvenOdd(pForce->GetData(), m_fCoeffGamma3, GAMMA3, m_byFieldId);
    }
    if (abs(m_fCoeffGamma4) > _CLG_FLT_EPSILON)
    {
        _FermionKernelGamma::GammaKSForceEvenOdd(pForce->GetData(), m_fCoeffGamma4, GAMMA4, m_byFieldId);
    }
}

#if discard
void CFieldFermionKSSU3Gamma::GammaKSForceEvenOdd(
    void* pForce,
    Real fCoeff,
    EGammaMatrix eGamma,
    BYTE byFieldID)
{
    deviceSU3* forcesu3 = (deviceSU3*)(pForce);
    preparethread;
    ...
    case GAMMA4:
    {
        ...
        _kernelFermionKSForceGamma1234EvenOdd << <block, threads >> > (forcesu3, iDir, fCoeff, byFieldID);
    }
    break;
    case SIGMA12:
    case SIGMA31:
...
void CFieldFermionKSSU3Gamma::InitialOtherParameters(CParameters & params)
{
    ...
    if (params.FetchValueINT(_T("Imagine"), iValue))
    {
        ...
        if (!m_bImagine)
        {
            appWarning(_T("NOTE: Imagine is set to be FALSE, the M.C. has sign problem!!! Do this only in measure!!!\n"));
        }
    }
...
void CFieldFermionKSSU3Gamma::CopyParamTo(CField* U) const
{
...
void CFieldFermionKSSU3Gamma::ApplyGammaS(const CFieldGauge* pGauge, EGammaMatrix eGamma)
{
    ...
    CFieldFermionKSSU3* pPooled = dynamic_cast<CFieldFermionKSSU3*>(appGetLattice()->GetPooledFieldById(m_byFieldId, _T(__FILE__), __LINE__));
    ...
    _FermionKernelGamma::appApplyGammaKS(
        m_pDeviceData,
