    ...
    , m_pGaugeFixing(NULL)
{
    ...
    memset(m_pGaugeSmearing, 0, sizeof(CGaugeSmearing*) * kMaxFieldCount);
    memset(m_pStapleCaches, 0, sizeof(CStapleCache*) * kMaxFieldCount);
}
...
CLatticeData::~CLatticeData()
{
    ...
    {
        checkCudaErrors(__cudaFree(m_pDeviceRandom));
        m_pDeviceRandom = NULL;
    }
    ...
    for (BYTE byField = 0; byField < kMaxFieldCount; ++byField)
    {
        ...
        appSafeDelete(m_pGaugeSmearing[byField]);
        appSafeDelete(m_pStapleCaches[byField]);
    }
...
CField* CLatticeData::GetPooledFieldById(BYTE byId, const char* file, INT iLine)
{
    //const CField* orignalfield = GetFieldById(byId);
    //appDetailed(_T("Get Pooled Field ID: %d, %s(%d)\n"), byId, file, iLine);
    return appGetFieldPool()->GetOne(GetFieldById(byId));
}

CField* CLatticeData::GetPooledCopy(const CField* pField, const char* file, INT iLine)
{
    //appDetailed(_T("Get Pooled Field ID: %d, %s(%d)\n"), pField->m_byFieldId, file, iLine);
    CField* pooled = appGetFieldPool()->GetOne(pField);
    if (NULL != pooled)
    {
...
void CLatticeData::OnUpdatorConfigurationAccepted(INT gaugeNum, INT bosonNum, INT tensor2Num, const CFieldGauge* const* pAcceptGauge, const CFieldBoson* const* pAcceptBoson, const CFieldTensor2* const* pAcceptTensor2, const CFieldGauge* const* pCorrespondingStaple) const
{
    //accept gauge already copy to m_pGaugeField in Updator, maybe change this behavour in the furture.
    if (NULL != m_pMeasurements)
    {
        m_pMeasurements->OnConfigurationAccepted(gaugeNum, bosonNum, tensor2Num, pAcceptGauge, pAcceptBoson, pAcceptTensor2, pCorrespondingStaple);
    }
}
...
CCString CLatticeData::GetInfos(const CCString& sTab) const
{
    ...
    sRet = sRet + sTab + sInfos;
    sInfos.Format(_T("Exponential Precision : %d\n"), _HC_ExpPrecision);
    sRet = sRet + sTab + sInfos;
    ...
    for (INT i = 0; i < kMaxFieldCount; ++i)
    {
        ...
        if (NULL != m_pGaugeSmearing[i])
        {
            sRet = sRet + sTab + _T("Gauge Smearing : \n");
            sRet = sRet + m_pGaugeSmearing[i]->GetInfos(sTab + _T("    "));
        }
    }
    ...
    sInfos.Format(_T("All Boundary Field Count : %d\n"), m_pAllBoundaryFields.Num());
    for (INT i = 0; i < m_pAllBoundaryFields.Num(); ++i)
    {
        sRet = sRet + sTab + _T("Boundary Field") + appToString(i) + _T(" : \n");
        sRet = sRet + m_pAllBoundaryFields[i]->GetInfos(sTab + _T("    "));
    }
