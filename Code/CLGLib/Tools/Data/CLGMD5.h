...
*
*/
static DWORD* FoldDataMD5(UINT& iBlockCount, const BYTE* pInData, UINT iDataCount)
{
    const UINT iLength = (UINT)((INT)(iDataCount >> 6) + 1) * 16; //blocks is parted into every 64 chars, and so 16 DWORDs as a block
    DWORD* retVal = (DWORD*)malloc(sizeof(DWORD) * iLength);
    assert(retVal);
    memset(retVal, 0, sizeof(DWORD) * iLength);
    iBlockCount = iLength;
    ...
    for (UINT i = 0; i < iDataCount; ++i)
    {
        ...
        retVal[i >> 2] |= (DWORD)pInData[i] << (iMove * 8);
    }
    ...
    DWORD* pNewsValue = (DWORD*)malloc(sizeof(DWORD) * iNewLength);
    memset(pNewsValue, 0, sizeof(DWORD) * iNewLength);
    memcpy(pNewsValue, retVal, sizeof(DWORD) * iLength);
...
static DWORD* MD5HashFoldeded(DWORD* pFoldedData, UINT iFoldCount)
{
    /*
    ...
    */
    ...
    DWORD* pOutValue = (DWORD*)malloc(sizeof(DWORD) * 4);
    assert(pOutValue);
    ...
    for (UINT i = 0; i < (iFoldCount >> 4); ++i)
    {
        ...
        memcpy(x, pFoldedData + 16 * i, sizeof(DWORD) * 16);
...
{
    UINT iBlockCount = 0;
    DWORD* folded = FoldDataMD5(iBlockCount, pData, uiDataCount);
    DWORD* pResoult = MD5HashFoldeded(folded, iBlockCount);
