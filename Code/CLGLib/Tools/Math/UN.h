...
// 
// DESCRIPTION:
// N complex unitary matrix (not necessarily SU(N)).
// This is helper functions for calculate D=N complex matrix
// NOTE: It is not necessarily a UN
//
// 
// for U4
// 0, 1, 2, 3
...
public:
    ...
    __device__ deviceUN(const deviceUN<N, NoE>& other)
    {
        memcpy(m_me, other.m_me, sizeof(CLGComplex) * N * N);
    }

    __device__ deviceUN(const CLGComplex* other)
    {
...
#pragma region creation
    ...
    * ret = 0
    */
    __device__ __inline__ static deviceUN<N, NoE> makeUNZero()
    {
        deviceUN<N, NoE> ret;
        for (INT i = 0; i < N * N; ++i)
        {
    ...
    * ret = I
    */
    __device__ __inline__ static deviceUN<N, NoE> makeUNId()
    {
        deviceUN<N, NoE> ret;
        for (INT i = 0; i < N; ++i)
        {
    ...
    __device__ __inline__ static deviceUN<N, NoE> makeUNGenerator(UINT uiGenerator)
    {
        deviceUN<N, NoE> ret = makeUNZero();
        ...
        else if (uiGenerator == N * N - 1)
        {
            // Identity generator (U(1) phase)
            for (INT i = 0; i < N; ++i)
            {
                ret.m_me[i * N + i] = _make_cuComplex(F(1.0), F(0.0));
            }
        }
    ...
    *
    */
    __device__ __inline__ static deviceUN<N, NoE> makeUNRandomGenerator(UINT fatIndex)
    {
        deviceUN<N, NoE> ret = makeUNZero();
        for (UINT i = 0; i < N * N; ++i)
        {
            const Real r = _deviceRandomGaussFSqrt2(fatIndex);
            ret.Add(makeUNGenerator(i).MulCompC(_make_cuComplex(F(0.0), r)));
        }
        return ret;
    }

    __device__ __inline__ static deviceUN<N, NoE> makeUNSumGenerator(Real fDivide)
    {
        deviceUN<N, NoE> ret = makeUNZero();
        for (UINT i = 0; i < N * N; ++i)
        {
            ret.Add(makeUNGenerator(i).MulRealC(fDivide));
        }
        return ret;
    }
    ...
    * ret = random
    */
    __device__ __inline__ static deviceUN<N, NoE> makeUNRandom(UINT fatIndex)
    {
        deviceUN<N, NoE> ret;
        for (UINT i = 0; i < N * N; ++i)
        {
    ...
    /**
    * a random matrix which does not has to be U(N)
    */
    __device__ __inline__ static deviceUN<N, NoE> makeUNRandomAny(UINT fatIndex)
    {
        deviceUN<N, NoE> ret;
        for (UINT i = 0; i < N * N; ++i)
        {
    ...
    template<INT NoVE>
    __device__ __inline__ static deviceUN<N, NoE> makeUNContractV(const deviceSUNVector<N, NoVE>& left, const deviceSUNVector<N, NoVE>& right)
    {
        deviceUN<N, NoE> ret;
        for (INT y = 0; y < N; ++y)
        {
    ...
    __device__ __inline__ static deviceUN<N, NoE> makeUNContractV(const deviceSU3Vector& left, const deviceSU3Vector& right)
    {
        if constexpr (3 == N)
        {
            deviceUN<N, NoE> ret;
        ...
        return deviceUN<N, NoE>::makeUNZero();
    }

    __device__ __inline__ static deviceUN<N, NoE> makeUNContractV(const deviceSU2Vector& left, const deviceSU2Vector& right)
    {
        if constexpr (2 == N)
        {
            deviceUN<N, NoE> ret;
...
#pragma region operators
    ...
    __device__ __inline__ void Add(const deviceUN<N, NoE>& right)
    {
        for (INT i = 0; i < N * N; ++i)
        {
    ...
    __device__ __inline__ void AddDagger(const deviceUN<N, NoE>& right)
    {
        for (INT y = 0; y < N; ++y)
        {
            for (INT x = 0; x < N; ++x)
    ...
    __device__ __inline__ void Sub(const deviceUN<N, NoE>& right)
    {
        for (INT i = 0; i < N * N; ++i)
        {
    ...
    __device__ __inline__ void SubDagger(const deviceUN<N, NoE>& right)
    {
        for (INT y = 0; y < N; ++y)
        {
            for (INT x = 0; x < N; ++x)
    ...
    __device__ __inline__ void Mul(const deviceUN<N, NoE>& right)
    {
        CLGComplex temp[N];
        for (INT y = 0; y < N; ++y)
        {
    ...
    __device__ __inline__ void Mul(const deviceUN<N, NoE>& right, CLGComplex* temp)
    {
        for (INT y = 0; y < N; ++y)
        {
            for (INT x = 0; x < N; ++x)
    ...
    __device__ __inline__ void DaggerMul(const deviceUN<N, NoE>& right)
    {
        Dagger();
        Mul(right);
    }

    __device__ __inline__ void MulDagger(const deviceUN<N, NoE>& right)
    {
        CLGComplex temp[N];
        for (INT y = 0; y < N; ++y)
        {
    ...
    __device__ __inline__ void MulDagger(const deviceUN<N, NoE>& right, CLGComplex* temp)
    {
        for (INT y = 0; y < N; ++y)
        {
            for (INT x = 0; x < N; ++x)
    ...
    __device__ __inline__ void MulOnMe(const deviceUN<N, NoE>& left)
    {
        CLGComplex temp[N];
        for (INT x = 0; x < N; ++x)
        {
    ...
    __device__ __inline__ void MulOnMeDN(const deviceUN<N, NoE>& left)
    {
        CLGComplex temp[N];
        for (INT x = 0; x < N; ++x)
        {
    ...
    __device__ __inline__ void MulOnMeND(const deviceUN<N, NoE>& left)
    {
        Dagger();
        MulOnMe(left);
    }

    __device__ __inline__ deviceUN<N, NoE> MulC(const deviceUN<N, NoE>& right) const
    {
        deviceUN<N, NoE> ret;
        for (INT x = 0; x < N; ++x)
        {
    ...
    __device__ __inline__ deviceUN<N, NoE> MulDaggerC(const deviceUN<N, NoE>& right) const
    {
        deviceUN<N, NoE> ret;
        for (INT x = 0; x < N; ++x)
        {
    ...
    __device__ __inline__ deviceUN<N, NoE> DaggerMulC(const deviceUN<N, NoE>& right) const
    {
        deviceUN<N, NoE> ret;
        for (INT x = 0; x < N; ++x)
        {
    ...
    __device__ __inline__ deviceUN<N, NoE> AddC(const deviceUN<N, NoE>& right) const { deviceUN<N, NoE> ret(*this); ret.Add(right); return ret; }
    __device__ __inline__ deviceUN<N, NoE> AddCompC(const CLGComplex& right) const { deviceUN<N, NoE> ret(*this); ret.AddComp(right); return ret; }
    __device__ __inline__ deviceUN<N, NoE> AddRealC(const Real& right) const { deviceUN<N, NoE> ret(*this); ret.AddReal(right); return ret; }

    __device__ __inline__ deviceUN<N, NoE> SubC(const deviceUN<N, NoE>& right) const { deviceUN<N, NoE> ret(*this); ret.Sub(right); return ret; }
    __device__ __inline__ deviceUN<N, NoE> SubCompC(const CLGComplex& right) const { deviceUN<N, NoE> ret(*this); ret.SubComp(right); return ret; }
    __device__ __inline__ deviceUN<N, NoE> SubRealC(const Real& right) const { deviceUN<N, NoE> ret(*this); ret.SubReal(right); return ret; }

    __device__ __inline__ deviceUN<N, NoE> MulCompC(const CLGComplex& right) const { deviceUN<N, NoE> ret(*this); ret.MulComp(right); return ret; }
    __device__ __inline__ deviceUN<N, NoE> MulRealC(const Real& right) const { deviceUN<N, NoE> ret(*this); ret.MulReal(right); return ret; }
    __device__ __inline__ deviceUN<N, NoE> DivCompC(const CLGComplex& right) const { deviceUN<N, NoE> ret(*this); ret.DivComp(right); return ret; }
    __device__ __inline__ deviceUN<N, NoE> DivRealC(const Real& right) const { deviceUN<N, NoE> ret(*this); ret.DivReal(right); return ret; }

    /**
    * self block mult must be square
    */
    __device__ __inline__ void BlockMul(
        const deviceUN<N, NoE>& right,
        UINT iWidth, 
    ...
    __device__ __inline__ void BlockCopy(const deviceUN<N, NoE>& right,
        UINT iLeftStartX, UINT iLeftStartY,
        UINT iRightStartX, UINT iRightStartY,
...
public:
    ...
    */
    ...
    __device__ __inline__ deviceUN<N, NoE> ReC() const { deviceUN<N, NoE> ret(*this); ret.Re(); return ret; }
    ...
    __device__ __inline__ deviceUN<N, NoE> ImC() const { deviceUN<N, NoE> ret(*this); ret.Im(); return ret; }
    ...
    */
    ...
    __device__ __inline__ deviceUN<N, NoE> DaggerC() const
    {
        deviceUN<N, NoE> ret;
        for (INT x = 0; x < N; ++x)
        {
    ...
    __device__ __inline__ deviceUN<N, NoE> OppositeC() const
    {
        deviceUN<N, NoE> ret(*this);
        ret.Opposite();
        return ret;
    }

    __device__ __inline deviceUN<N, NoE> Transpose() const
    {
        deviceUN<N, NoE> ret;
        for (INT x = 0; x < N; ++x)
        {
    ...
    */
    ...
    __device__ __inline__ void Th()
    {
        ...
        for (INT x = 0; x < N; ++x)
        {
            m_me[x * N + x].x = m_me[x * N + x].x - trace;
        }
    }
    ...
    */
    __device__ __inline__ static Real TrIm(const deviceUN<N, NoE>& a, const deviceUN<N, NoE>& b)
    {
        Real ret = F(0.0);
        for (BYTE i = 0; i < N; ++i)
        {
            for (BYTE j = 0; j < N; ++j)
            {
                ret += a.m_me[i * N + j].y * b.m_me[j * N + i].y;
            }
        }
        return ret;
    }
    ...
    */
    __device__ __inline__ void iIm2()
    {
        for (BYTE i = 0; i < N; ++i)
        {
            ...
            {
                m_me[i * N + j] = _cuCsubf(m_me[i * N + j], _cuConjf(m_me[j * N + i]));
                m_me[j * N + i] = _make_cuComplex(-_cuCrealf(m_me[i * N + j]), _cuCimagf(m_me[i * N + j]));
            }
        }

        for (BYTE i = 0; i < N; ++i)
        {
            m_me[i * N + i] = _make_cuComplex(F(0.0), F(2.0) * m_me[i * N + i].y);
        }
    }
    ...
    __device__ __inline__ void Re2()
    {
        for (BYTE i = 0; i < N; ++i)
        {
            for (BYTE j = i + 1; j < N; ++j)
            {
                m_me[i * N + j] = _cuCaddf(m_me[i * N + j], _cuConjf(m_me[j * N + i]));
                m_me[j * N + i] = _make_cuComplex(_cuCrealf(m_me[i * N + j]), -_cuCimagf(m_me[i * N + j]));
            }
        }
        ...
        for (BYTE i = 0; i < N; ++i)
        {
            m_me[i * N + i] = _make_cuComplex(F(2.0) * m_me[i * N + i].x, F(0.0));
        }
    }
    ...
    */
    //__device__ __inline__ deviceU3 Im2C() const
    //{
    //    deviceU3 ret;
    //    //0 1 2
    //    //3 4 5
    ...
    * let M = {{0.9879009 + 0.0051302 I, -0.0682856 - 0.0641515 I, -0.0821236 - 0.0152904 I, -0.0859499 - 0.0296949 I}, 
      ...
      but, Q/Exp[-I Arg[Det[Q]]/4], although was a UN matrix, but not M (which is already a UN matrix)
    */
    ...
    __device__ __inline__ void Proj(BYTE ite = N + 1)
    {
        ...
        for (BYTE byIt = 0; byIt < ite; ++byIt)
        {
            ...
            // U(N) does not require det = 1, so no phase correction needed
        }
    }

    /**
     * Cabbibo-Marinari Projection
     * Only one iteration can make it U(N), however, 6 iteration makes it large trace
    ...
    */
    __device__ __inline__ deviceUN<N, NoE> Exp(const CLGComplex& a, BYTE uiPrecision = N + 1) const
    {
        deviceUN<N, NoE> tmp;
    ...
    __device__ __inline__ deviceUN<N, NoE> ExpReal(Real a, BYTE uiPrecision = N + 1) const
    {
        deviceUN<N, NoE> tmp;
        for (BYTE i = 0; i < uiPrecision; ++i)
        {
...
private:
    ...
    * 
    */
    __device__ __inline__ void OneStepHouseHolderQR(deviceUN<N, NoE>& A, UINT i)
    {
...
public:
    ...
    */
    __device__ __inline__ deviceUN<N, NoE> QR()
    {
        deviceUN<N, NoE> r(*this);
...
private:
    ...
    __device__ __inline__ void OneStepGaussianLU(deviceUN<N, NoE>& L, UINT i)
    {
...
public:
    ...
    */
    __device__ __inline__ deviceUN<N, NoE> LU()
    {
        deviceUN<N, NoE> L = makeUNId();
        for (UINT i = 0; i < (N - 1); ++i)
        {
...
private:
    ...
    __device__ __inline__ void OneStepHouseHolderHessenberg(deviceUN<N, NoE>& A, UINT i)
    {
        //Calculate beta and v
        Real fLength = F(0.0);
        for (INT y = i + 1; y < N; ++y)
    ...
    __device__ __inline__ void OneStepHouseHolderHessenbergC(deviceUN<N, NoE>& A, UINT i) const
    {
        //Calculate beta and v
        Real fLength = F(0.0);
        for (INT y = i + 1; y < N; ++y)
...
public:
    ...
    __device__ __inline__ deviceUN<N, NoE> Hessenberg()
    {
        deviceUN<N, NoE> r(*this);
    ...
    __device__ __inline__ deviceUN<N, NoE> HessenbergC() const
    {
        deviceUN<N, NoE> r(*this);
        for (UINT i = 0; i < (N - 2); ++i)
        {
...
private:
    ...
    //__device__ __inline__ void InversePower(const CLGComplex& ev, CLGComplex* startV) const
    {
        ...
        INT iMaxIte = N;
        CLGComplex tmp[N];
        const deviceUN<N, NoE> m = SubCompC(ev);
        //m.DebugPrint("m");
        deviceUN<N, NoE> q(m);
        const deviceUN<N, NoE> r = q.QR();
        q.Dagger();
        CLGComplex mv[N];
...
public:
    ...
    {
        deviceUN<N, NoE> tmp = HessenbergC();
    ...
    __device__ __inline__ deviceUN<N, NoE> EigenSystem(CLGComplex* evs) const
    {
        EigenValues(evs);
        CLGComplex tmpv[N];

        deviceUN<N, NoE> eigenVectors;
        for (INT i = 0; i < N; ++i)
        {
            for (INT j = 0; j < N; ++j)
            {
    ...
    __device__ __inline__ deviceUN<N, NoE> Log() const
    {
        CLGComplex eign[N];
        deviceUN<N, NoE> egv = EigenSystem(eign);
    ...
    __device__ __inline__ deviceUN<N, NoE> StrictExp() const
    {
        CLGComplex eign[N];
        deviceUN<N, NoE> egv = EigenSystem(eign);
    ...
    __device__ __inline__ deviceUN<N, NoE> Power(Real fpower) const
    {
        CLGComplex eign[N];
        deviceUN<N, NoE> egv = EigenSystem(eign);
        ...
        for (INT i = 0; i < N; ++i)
        {
            ret.m_me[i * N + i] = __cuCpowerf(eign[i], fpower);//__cuClogf(eign[i]);
        }
        ret.MulDagger(egv);
        return egv.MulC(ret);
    }

    __device__ __inline__ deviceUN<N, NoE> Inverse() const
    {
        return Power(F(-1.0));
    }
...
//typedef deviceUN<4, 16> deviceU4;
//typedef deviceUN<5, 32> deviceU5;
//typedef deviceUN<6, 64> deviceU6;
//typedef deviceUN<7, 64> deviceU7;
//typedef deviceUN<8, 64> deviceU8;

#define _TYPEDEFUN(n, moe) typedef deviceUN<n, moe> deviceU##n;
#define _DEF_F2_TO_UN(n, imp) _DEF_F2UN_N(n, imp, 256, 256, 256, 256, 256, 128, 128, 128, 64, 64, 64, 32, 16, 16, 4)

_DEF_F2_TO_UN(_MAX_UN, _TYPEDEFUN)
