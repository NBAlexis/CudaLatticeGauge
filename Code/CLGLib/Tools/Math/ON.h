...
//
// DESCRIPTION:
// This is helper functions for calculate O(N) real orthogonal matrix.
// O(N) = N x N real matrices with R^T R = I, det = +/-1.
//
// Layout:
// for N=4
// 0, 1, 2, 3
...
public:
    ...
    __device__ deviceON(const deviceON<N, NoE>& other)
    {
        memcpy(m_me, other.m_me, sizeof(Real) * N * N);
    }

    __device__ deviceON(const Real* other)
    {
...
#pragma region creation
    ...
    __device__ __inline__ static deviceON<N, NoE> makeONZero()
    {
        deviceON<N, NoE> ret;
        for (INT i = 0; i < N * N; ++i)
        {
    ...
    __device__ __inline__ static deviceON<N, NoE> makeONId()
    {
        deviceON<N, NoE> ret;
        for (INT i = 0; i < N; ++i)
        {
    ...
    */
    __device__ __inline__ static deviceON<N, NoE> makeONGenerator(UINT uiGenerator)
    {
        deviceON<N, NoE> ret = makeONZero();
        UINT count = 0;
        for (UINT i = 0; i < N; ++i)
        {
    ...
    */
    __device__ __inline__ static deviceON<N, NoE> makeONRandomGenerator(UINT fatIndex)
    {
        deviceON<N, NoE> ret = makeONZero();
        for (UINT i = 0; i < N * (N - 1) / 2; ++i)
        {
            const Real r = _deviceRandomGaussFSqrt2(fatIndex);
            ret.Add(makeONGenerator(i).MulRealC(r));
        }
        return ret;
    }

    __device__ __inline__ static deviceON<N, NoE> makeONSumGenerator(Real fDivide)
    {
        deviceON<N, NoE> ret = makeONZero();
        for (UINT i = 0; i < N * (N - 1) / 2; ++i)
        {
            ret.Add(makeONGenerator(i).MulRealC(fDivide));
        }
        return ret;
    }

    __device__ __inline__ static deviceON<N, NoE> makeONRandom(UINT fatIndex)
    {
        deviceON<N, NoE> ret;
    ...
    __device__ __inline__ static deviceON<N, NoE> makeONRandomAny(UINT fatIndex)
    {
        deviceON<N, NoE> ret;
        for (UINT i = 0; i < N * N; ++i)
        {
...
#pragma region operators
    ...
    __device__ __inline__ void Add(const deviceON<N, NoE>& right)
    {
        for (INT i = 0; i < N * N; ++i)
        {
            m_me[i] += right.m_me[i];
    ...
    __device__ __inline__ void Sub(const deviceON<N, NoE>& right)
    {
        for (INT i = 0; i < N * N; ++i)
        {
            m_me[i] -= right.m_me[i];
    ...
    __device__ __inline__ void Mul(const deviceON<N, NoE>& right)
    {
        Real temp[N];
        for (INT y = 0; y < N; ++y)
        {
    ...
    __device__ __inline__ void Mul(const deviceON<N, NoE>& right, Real* temp)
    {
        for (INT y = 0; y < N; ++y)
        {
            for (INT x = 0; x < N; ++x)
    ...
    __device__ __inline__ void TransposeMul(const deviceON<N, NoE>& right)
    {
        Transpose();
        Mul(right);
    }

    __device__ __inline__ void MulTranspose(const deviceON<N, NoE>& right)
    {
        Real temp[N];
        for (INT y = 0; y < N; ++y)
        {
    ...
    __device__ __inline__ void MulTranspose(const deviceON<N, NoE>& right, Real* temp)
    {
        for (INT y = 0; y < N; ++y)
        {
            for (INT x = 0; x < N; ++x)
    ...
    __device__ __inline__ void MulOnMe(const deviceON<N, NoE>& left)
    {
        Real temp[N];
        for (INT x = 0; x < N; ++x)
        {
    ...
    __device__ __inline__ deviceON<N, NoE> MulC(const deviceON<N, NoE>& right) const
    {
        deviceON<N, NoE> ret;
        for (INT x = 0; x < N; ++x)
        {
    ...
    __device__ __inline__ deviceON<N, NoE> MulTransposeC(const deviceON<N, NoE>& right) const
    {
        deviceON<N, NoE> ret;
        for (INT x = 0; x < N; ++x)
        {
    ...
    __device__ __inline__ deviceON<N, NoE> TransposeMulC(const deviceON<N, NoE>& right) const
    {
        deviceON<N, NoE> ret;
        for (INT x = 0; x < N; ++x)
        {
    ...
    __device__ __inline__ deviceON<N, NoE> AddC(const deviceON<N, NoE>& right) const { deviceON<N, NoE> ret(*this); ret.Add(right); return ret; }
    __device__ __inline__ deviceON<N, NoE> AddRealC(const Real& right) const { deviceON<N, NoE> ret(*this); ret.AddReal(right); return ret; }

    __device__ __inline__ deviceON<N, NoE> SubC(const deviceON<N, NoE>& right) const { deviceON<N, NoE> ret(*this); ret.Sub(right); return ret; }
    __device__ __inline__ deviceON<N, NoE> SubRealC(const Real& right) const { deviceON<N, NoE> ret(*this); ret.SubReal(right); return ret; }

    __device__ __inline__ deviceON<N, NoE> MulRealC(const Real& right) const { deviceON<N, NoE> ret(*this); ret.MulReal(right); return ret; }
    __device__ __inline__ deviceON<N, NoE> DivRealC(const Real& right) const { deviceON<N, NoE> ret(*this); ret.DivReal(right); return ret; }

    __device__ __inline__ void BlockMul(
        const deviceON<N, NoE>& right,
        UINT iWidth,
    ...
    __device__ __inline__ void BlockCopy(const deviceON<N, NoE>& right,
        UINT iLeftStartX, UINT iLeftStartY,
        UINT iRightStartX, UINT iRightStartY,
    ...
    template<INT NoVE>
    __device__ __inline__ static deviceON<N, NoE> makeONContractV(const deviceSUNVector<N, NoVE>& left, const deviceSUNVector<N, NoVE>& right)
    {
        deviceON<N, NoE> ret;
        for (INT y = 0; y < N; ++y)
        {
    ...
    __device__ __inline__ static deviceON<N, NoE> makeONContractV(const deviceSU3Vector& left, const deviceSU3Vector& right)
    {
        if constexpr (3 == N)
        {
            deviceON<N, NoE> ret;
        ...
        return deviceON<N, NoE>::makeONZero();
    }

    __device__ __inline__ static deviceON<N, NoE> makeONContractV(const deviceSU2Vector& left, const deviceSU2Vector& right)
    {
        if constexpr (2 == N)
        {
            deviceON<N, NoE> ret;
...
public:
    ...
    __device__ __inline__ deviceON<N, NoE> TransposeC() const
    {
        deviceON<N, NoE> ret;
        for (INT x = 0; x < N; ++x)
        {
    ...
    __device__ __inline__ deviceON<N, NoE> OppositeC() const
    {
        deviceON<N, NoE> ret(*this);
        ret.Opposite();
        return ret;
    }
    ...
    */
    __device__ __inline__ deviceON<N, NoE> ExpReal(Real a, BYTE uiPrecision = N + 1) const
    {
        deviceON<N, NoE> tmp;
        for (BYTE i = 0; i < uiPrecision; ++i)
        {
            deviceON<N, NoE> aUoN(*this);
    ...
    /**
     * Approximate log for orthogonal matrices: log(O) ~= (O - O^T)/2
     * Exact for O near identity.
     */
    __device__ __inline__ deviceON<N, NoE> Log() const
    {
        deviceON<N, NoE> res(*this);
        res.Ta();
        return res;
    }
...
private:
    ...
    __device__ __inline__ void OneStepHouseHolderQR(deviceON<N, NoE>& A, UINT i)
    {
        Real fLength = F(0.0);
        for (INT y = i; y < N; ++y)
        {
...
public:
    ...
    __device__ __inline__ deviceON<N, NoE> QR()
    {
        deviceON<N, NoE> r(*this);
...
private:
    ...
    __device__ __inline__ void OneStepGaussianLU(deviceON<N, NoE>& L, UINT i)
    {
...
public:
    ...
    __device__ __inline__ deviceON<N, NoE> LU()
    {
        deviceON<N, NoE> L = makeONId();
        for (UINT i = 0; i < (N - 1); ++i)
        {
...
private:
    ...
    __device__ __inline__ void OneStepHouseHolderHessenberg(deviceON<N, NoE>& A, UINT i)
    {
        Real fLength = F(0.0);
        for (INT y = i + 1; y < N; ++y)
        {
    ...
    __device__ __inline__ void OneStepHouseHolderHessenbergC(deviceON<N, NoE>& A, UINT i) const
    {
        Real fLength = F(0.0);
        for (INT y = i + 1; y < N; ++y)
        {
...
public:
    ...
    __device__ __inline__ deviceON<N, NoE> Hessenberg()
    {
        deviceON<N, NoE> r(*this);
    ...
    __device__ __inline__ deviceON<N, NoE> HessenbergC() const
    {
        deviceON<N, NoE> r(*this);
        for (UINT i = 0; i < (N - 2); ++i)
        {
...
public:
    ...
    {
        deviceON<N, NoE> tmp = HessenbergC();
...
#define _TYPEDEFON(n, moe) typedef deviceON<n, moe> deviceO##n;
#define _DEF_F2_TO_ON(n, imp) _DEF_F2ON_N(n, imp, 256, 256, 256, 256, 256, 128, 128, 128, 64, 64, 64, 32, 16, 16, 4)

_DEF_F2_TO_ON(_MAX_ON, _TYPEDEFON)
