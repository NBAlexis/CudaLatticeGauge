    ...
    EFieldOperator uiM, ESolverPhase ePhase, const CField* pStart)
{
    ...
    CField* pT = appGetLattice()->GetPooledFieldById(pFieldB->m_byFieldId, _T(__FILE__), __LINE__);
    ...
    for (UINT i = 0; i < m_uiReTry; ++i)
    {
        //appGeneral(_T(" ============================ pR ===================\n"));
        //pR->DebugPrintMe();
        pR->ApplyOperator(uiM, gaugeNum, bosonNum, tensor2Num, gaugeFields, bosonFields, tensor2Fields, EOCT_Minus); //-A x_0
        //appGeneral(_T(" ============================ pR2 ===================\n"));
        //pR->DebugPrintMe();
        ...
        //After many test, if looks rh = r works bettter
        //pRh->Dagger();
        pR->CopyTo(pP);
        ...
        for (UINT j = 0; j < m_uiStepCount * m_uiDevationCheck; ++j)
        {
...
#else
            if (cuCabsSq(rho) < ((m_uiReTry == i + 1) ? _CLG_FLT_MIN : m_fSmallRho))
            {
                appParanoiac(_T("CSLASolverBiCGStab::rho too small:%0.28f\n"), cuCabsSq(rho));
#endif
                i = i + 1;
            ...
            //v(i) = A p(i)
            pP->CopyTo(pV);
            pV->ApplyOperator(uiM, gaugeNum, bosonNum, tensor2Num, gaugeFields, bosonFields, tensor2Fields);
#if !_CLG_DOUBLEFLOAT
            ...
            if (0 == (j + 1) % m_uiDevationCheck)
            {
                ...
                const DOUBLE fDeviation = pS->GetLength() / fBLength;
            ...
            //t=As
            pS->CopyTo(pT);
            pT->ApplyOperator(uiM, gaugeNum, bosonNum, tensor2Num, gaugeFields, bosonFields, tensor2Fields);

#if !_CLG_DOUBLEFLOAT
            omega = cuCdivf_cd_host(pS->Dot(pT), pT->GetLength());//omega = ts / tt
#else
            omega = cuCdivf_cr_host(pS->Dot(pT), pT->GetLength());//omega = ts / tt
#endif
