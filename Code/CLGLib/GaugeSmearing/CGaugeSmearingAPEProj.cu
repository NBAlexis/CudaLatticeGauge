...
#include "CGaugeSmearingAPEProj.h"
...
void CGaugeSmearingAPEProj::GaugeSmearing(CFieldGauge* pGauge, const CFieldGauge* pOrignal, CFieldGauge* pStaple, UBOOL bProject)
{
    ...
    for (UINT i = 0; i < m_uiIterate; ++i)
    {
        ...
        if (bProject)
        {
            if (m_bCMProj)
            {
                _LAUNCH_KERNEL(_kernelAPEProjSU3CM, block, threads, 
            ...
            {
                _LAUNCH_KERNEL(_kernelAPEProjSU3, block, threads, 
                    pGaugeSU3->m_pDeviceData,
                    pStapleSU3->m_pDeviceData,
                    m_fAlphaLeft, m_fAlphaRight,
                    byMaxLink,
...
CCString CGaugeSmearingAPEProj::GetInfos(const CCString &tab) const
{
    ...
    sRet = sRet + tab + _T("alpha left : ") + appToString(m_fAlphaLeft) + _T("\n");
    sRet = sRet + tab + _T("alpha rihgt : ") + appToString(m_fAlphaRight) + _T("\n");
    sRet = sRet + tab + _T("CabibboMarinari : ") + (m_bCMProj ? _T("1\n") : _T("0\n"));
    sRet = sRet + tab + _T("ProjIterate : ") + appToString(static_cast<INT>(m_byProjIterate)) + _T("\n");
    return sRet;
}
