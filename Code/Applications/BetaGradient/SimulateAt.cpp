...
INT SimulateAtGradient(CParameters& params)
{
#pragma region Parameters
    ...
    TArray<DOUBLE> fXi;
    params.FetchValueArrayDOUBLE(_T("XiList"), fXi);
...
#pragma endregion
    ...
    if (fXi.Num() != _HC_Lzi)
    {
        appCrucial(_T("XiList not corrected!\n"));
        return 1;
    }
    ...
    CActionGaugePlaquetteAtGradient* pGaugeGradient = dynamic_cast<CActionGaugePlaquetteAtGradient*>(appGetLattice()->GetActionById(1));

    pGaugeGradient->SetBeta(fBeta[iListStart]);
    pGaugeGradient->SetXiList(fXi);
    ...
    for (INT uiOmega = iListStart; uiOmega < sPrefixList.Num() && uiOmega < iListEnd; ++uiOmega)
    {
        ...
        appGeneral(_T("\n========= Beta=%f  ==========\n"), fBeta[uiOmega]);
        pGaugeGradient->SetBeta(fBeta[uiOmega]);
        pGaugeGradient->SetXiList(fXi);
