...
INT SimulateAcc(CParameters& params)
{
    ...
    iVaule = 250;
    params.FetchValueINT(_T("AccSep"), iVaule);
    ...
    iVaule = 0;
    params.FetchValueINT(_T("AccStart"), iVaule);
    ...
    params.FetchValueReal(_T("MaxAcc"), fMaxOmega);
    ...
    for (UINT uiNt = iMinNt; uiNt <= iMaxNt; ++uiNt)
    {
        ...
        while (uiOmega <= iAfterEquib)
        {
            ...
            if (bAdditive)
            {
                ...
                appGetLattice()->m_pMeasurements->Reset();
                sFileName.Format(_T("%sAcc_Nt%d_O%d_%d.con"), sSavePrefix.c_str(), uiNt, uiOmega, iSaveStartIndex);
            ...
            while (iConfigNumberNow < iEquib)
            {
                ...
                {
                    sFileName.Format(_T("Acc_Nt%d_O%d_%d"), uiNt, uiOmega, uiAcce + iSaveStartIndex);
