...
INT SimulateNf2p1(CParameters& params)
{
...
#pragma endregion
    ...
    if (bNeedBake && iWarmUp > 0)
    {
        ...
        pGaugeAction->SetBeta(BetaList[iListStart]);
    ...
    while (uiOmega < BetaList.Num() && uiOmega < iListEnd)
    {
        ...
        if (bAdditive)
        {
            ...
            appGetLattice()->m_pMeasurements->Reset();
            sFileName.Format(_T("%sHISQScan_%s_%d.con"), sSavePrefix.c_str(), PrefixList[uiOmega].c_str(), iSaveStartIndex);
            ...
            {
                appGeneral(_T("\n ================ have the initial file, but not matching.... %2.12f, %2.12f, diff=%f ===========\n"),
                    static_cast<Real>(cuCabs(pPL->m_lstLoop[0])), fPolyaOld, fError);
                appFailQuitCLG();
                return 1;
            }
        }
        ...
        while (iConfigNumberNow < iEquib)
        {
            ...
            {
                sFileName.Format(_T("HISQScan_%s_%d"), PrefixList[uiOmega].c_str(), uiAcce + iSaveStartIndex);
