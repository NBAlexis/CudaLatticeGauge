...
for i in range(len(betalst)):
    wilsonloop = LoadMathematicaCSV(f"{folderstart}QuenchWilsonLoop\\DistanceFoldR\\BSQ_DistanceFoldR__{betalst[i]}_wilsonloops.csv")
...
np.savetxt(f'{folderstart}\\WilsonLoopRes\\BSQDistanceFoldR.csv', kalllst, delimiter=',')
